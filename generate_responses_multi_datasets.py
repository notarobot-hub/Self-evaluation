...
class MultiDatasetResponseGenerator:
    ...
    def setup_stopping_criteria(self):
        ...
        config = self.model_configs.get(self.model_name, self.model_configs["llama2-7b"])
        stop_words = config["stop_token"]
    ...
    def process_triviaqa(self, num_samples=100):
        ...
        try:
            ...
            for i, example in enumerate(dataset):
                ...
                print(f"Processed TriviaQA sample {i+1}/{min(num_samples, len(dataset))}")
                
            return results
            
        except Exception as e:
            print(f"Error processing TriviaQA: {e}")
            return []
    
    def process_truthfulqa(self, num_samples=100):
        ...
        try:
            ...
            for i, example in enumerate(dataset):
                ...
                print(f"Processed TruthfulQA sample {i+1}/{min(num_samples, len(dataset))}")
                
            return results
            
        except Exception as e:
            print(f"Error processing TruthfulQA: {e}")
            return []
    
    def process_coqa(self, num_samples=100):
        """Process CoQA dataset"""
        print("Processing CoQA dataset...")
        ...
        try:
            ...
            for i, example in enumerate(dataset):
                ...
                # Get first question from the conversation
                if example['questions'] and len(example['questions']) > 0:
                    ...
                    print(f"Processed CoQA sample {i+1}/{min(num_samples, len(dataset))}")
                
            return results
            
        except Exception as e:
            print(f"Error processing CoQA: {e}")
            return []
    
    def process_tydiqa(self, num_samples=100):
        """Process TyDiQA dataset"""
        print("Processing TyDiQA dataset...")
        
        try:
            # Load TyDiQA dataset
            dataset = load_dataset("tydiqa", "primary_task", split="train")
            ...
            for i, example in enumerate(dataset):
                ...
                print(f"Processed TyDiQA sample {i+1}/{min(num_samples, len(dataset))}")
                
            return results
            
        except Exception as e:
            print(f"Error processing TyDiQA: {e}")
            return []
    ...
    def run_all_datasets(self, num_samples_per_dataset=50):
        ...
        datasets = [
            ('triviaqa', self.process_triviaqa),
            ('truthfulqa', self.process_truthfulqa),
            ('coqa', self.process_coqa),
            ('tydiqa', self.process_tydiqa)
        ]
        ...
        for dataset_name, process_func in datasets:
            ...
            try:
                results = process_func(num_samples_per_dataset)
                all_results.extend(results)
                
                # Save intermediate results
                intermediate_file = f"./data/{self.model_name}_{dataset_name}_responses.json"
        ...
        # Save all results
        all_results_file = f"./data/{self.model_name}_all_datasets_responses.json"
...
def main():
    ...
    # Use provided model_path if specified, otherwise use default
    model_path = args.model_path if args.model_path != 'default' else model_paths.get(args.model_name)
    
    if not model_path:
        print(f"Error: No model path specified for {args.model_name}")
        return
    
    try:
        # Initialize generator
        generator = MultiDatasetResponseGenerator(
